...
if (!empty($_POST['btnCarga'])) {
    if (!empty($_POST['serial']) and !empty($_POST['modelo']) and !empty(['location']) and !empty(['date']) and !empty(['file'])){
        ...
        if ($sql==1) {
            echo '<div class="alert alert-success">LA CARGA SE REALIZO CORRECTAMENTE</div>';
        } else {
            echo '<div class="alert alert-danger">ERROR DE CONEXION COD: 10231514</div>';
        }
        
    } else{
        echo '<div class="alert alert-warning">DEBE LLENAR LOS CAMPOS VACIOS</div>';
    }
